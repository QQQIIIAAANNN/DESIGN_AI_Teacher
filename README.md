...
- **知識檢索與圖面審查**：按審查主題從平台準則及可用的私有教材索引檢索要點，再以證據評分和定位。缺少有效落點的意見會標示「位置待確認」，不會在圖上畫出猜測框；點選正確位置後可局部重審。
...
評分會按所選練習情境檢查核心主題的圖面證據，重大問題、缺少題目資料或尚未辨識的重要位置會限制暫評上限。若觸發上限，平台會同步調整五項分數，並在分數說明列出原因。
